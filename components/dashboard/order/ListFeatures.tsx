// src/components/dashboard/order/ListFeatures.tsx
'use client';
...
export default function ListFeatures({
  ...
  const handleToggleDiscussionStatus = (featureId: string, newStatus: 'open' | 'resolved' | 'archived') => {
    ...
    if (newStatus === 'resolved') {
      setAlertConfig({
        ...
        cancelText: 'Batal',
        onConfirm: () => {
          setDiscussionStatus({ ...discussionStatus, [featureId]: 'resolved' });
      ...
      setAlertConfig({
        ...
        cancelText: 'Batal',
        onConfirm: () => {
          setDiscussionStatus({ ...discussionStatus, [featureId]: 'archived' });
          setData({
            ...data,
            features: data.features.map((f) =>
          ...
          setOpenDiscussionNotes({ ...openDiscussionNotes, [featureId]: false });
          setAlertConfig((prev) => ({ ...prev, isOpen: false }));
        },
        onCancel: () => setAlertConfig((prev) => ({ ...prev, isOpen: false })),
      });
    } else {
      setDiscussionStatus({ ...discussionStatus, [featureId]: 'open' });
    }
  };

  const totalPrice = data.features.reduce((sum, feature) => sum + Number(feature.price), 0);
  ...
  const filteredFeatures = filterStatus === 'all' ? data.features : data.features.filter((f) => f.approvalStatus === filterStatus);
      ...
      <ul>
        ...
        {filteredFeatures.length === 0 ? (
          ...
          filteredFeatures.map((feature) => (
            <li key={feature.id} className="border-y border-gray-200 py-2 text-sm flex flex-col justify-between items-start">
              <div className="flex w-full justify-between space-y-2">
                <div className="space-y-1">
                  <div className="flex gap-2">
                    ...
                    {feature.isMarkedForDiscussion && discussionStatus[feature.id] !== 'archived' && (
                      <div className="inline-flex items-center gap-1 text-xs bg-red-50 border border-red-200 text-red-700 px-2 py-0.5 rounded-md">
                        ...
                        <button
                          onClick={() => setOpenDiscussionNotes({ ...openDiscussionNotes, [feature.id]: !openDiscussionNotes[feature.id] })}
                          className={`cursor-pointer underline ${
                            discussionStatus[feature.id] === 'resolved' ? 'text-blue-700 hover:text-blue-800' : 'text-red-700 hover:text-red-800'
                          }`}
                          disabled={readOnly}
                        >
                  ...
                  <p className="text-sm">Rp. {Number(feature.price).toLocaleString()} ({feature.duration})</p>
                  {feature.isMarkedForDiscussion && openDiscussionNotes[feature.id] && feature.discussionNote && discussionStatus[feature.id] !== 'archived' && (
                    <div
                      className={`mt-2 border-l-4 p-2 text-xs rounded ${
                        discussionStatus[feature.id] === 'resolved' ? 'bg-green-50 border-green-300 text-green-700' : 'bg-red-50 border-red-300 text-red-700'
                      }`}
                    >
                        ...
                        <div className="flex gap-2 mt-2">
                          <button
                            ...
                            className={`cursor-pointer text-xs underline ${
                              discussionStatus[feature.id] === 'resolved' ? 'text-green-700 hover:text-green-900' : 'text-red-700 hover:text-red-900'
                            }`}
                          >
                            {discussionStatus[feature.id] === 'resolved' ? 'Tandai Belum Selesai' : 'Tandai Selesai'}
                          </button>
              ...
              <div className="w-full mt-1">
                <div className="flex justify-between items-center text-xs font-medium text-gray-700">
                  <button
                    onClick={() => setOpenRequirements({ ...openRequirements, [feature.id]: !openRequirements[feature.id] })}
                    className="flex items-center gap-1 hover:text-gray-900 transition"
                  >
                    {openRequirements[feature.id] ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                    Detail Kebutuhan
                    <span className="text-gray-400 font-normal">({(feature.requirements || []).length})</span>
                  </button>
                  {feature.approvalStatus === 'approved' && (
                  ...
                  <div className="mt-2">
                    <ul className="space-y-1">
                      {(feature.requirements || []).map((req, index) => (
                        <li key={req.id} className="group flex items-center gap-1 justify-between text-xs rounded hover:bg-gray-50 transition">
                          <div className="flex items-center">
                            {!readOnly && feature.approvalStatus === 'draft' && (
                              <>
                                <button
                                ...
                                <button
                                  ...
                                  className={`p-1 rounded-full ${
                                    index === 0 ? 'text-gray-300 cursor-not-allowed' : 'text-gray-500 hover:bg-gray-200 group-hover:opacity-100 opacity-0 transition'
                                  }`}
                                  title="Naik"
                                >
                                  <ArrowUp size={12} />
                                ...
                                <button
                                  ...
                                  className={`p-1 rounded-full ${
                                    index === (feature.requirements?.length || 0) - 1
                                      ? 'text-gray-300 cursor-not-allowed'
                                      : 'text-gray-500 hover:bg-gray-200 group-hover:opacity-100 opacity-0 transition'
                                  }`}
                                  title="Turun"
                                >
                                  <ArrowDown size={12} />
                          ...
                          <div className="flex items-center flex-1 gap-2">
                            ...
                            {editingRequirement?.featureId === feature.id && editingRequirement?.requirementId === req.id ? (
                              <input
                                type="text"
                                ...
                                disabled={readOnly || feature.approvalStatus !== 'draft'}
                              />
                            ) : (
                              <span
                                className={`flex-1 text-gray-700 ${
                                  !readOnly && feature.approvalStatus === 'draft' ? 'hover:underline cursor-pointer' : 'cursor-not-allowed'
                                }`}
                                onClick={() => !readOnly && feature.approvalStatus === 'draft' && handleEditRequirement(feature.id, req.id, req.title)}
                              >
                                {req.title}
                              </span>
                            )}
                    ...
                    {!readOnly && feature.approvalStatus === 'draft' && (
                      <div className="flex gap-1 mt-3 items-center">
                        ...
                        <input
                          ...
                          onChange={(e) => setNewRequirement({ ...newRequirement, [feature.id]: e.target.value })}
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                              handleAddRequirement(feature.id, newRequirement[feature.id] || '');
                              setNewRequirement({ ...newRequirement, [feature.id]: '' });
